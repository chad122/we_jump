...
function create() {
  ...
  scene.render = function (ctx) {
    ...
    var d = this.data || {};
    draw.text(ctx, '本局结束', w / 2, 56, 30, '#ffffff', 'center', true);
    draw.text(ctx, d.reasonText || '', w / 2, 92, 14, 'rgba(255,255,255,0.7)');
    ...
    // 冠军横幅
    if (champSeat >= 0) {
      draw.fillRoundRect(ctx, w * 0.1, 118, w * 0.8, 54, 14, 'rgba(255,199,0,0.18)');
      draw.text(ctx, '★ 本局冠军', w / 2, 137, 20, '#ffd34d', 'center', true);
    }

    // 排名列表
    var ly = champSeat >= 0 ? 200 : 130;
    for (var i = 0; i < ranks.length; i++) {
      ...
      draw.text(ctx, String(r.rank), w * 0.13, py, 16, medalColor, 'left', true);
      ctx.fillStyle = state.seatColor(r.seat);
      ctx.beginPath();
      ctx.arc(w * 0.22, py, 7, 0, Math.PI * 2);
      ctx.fill();
